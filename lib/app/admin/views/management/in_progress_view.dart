...
class _InProgressViewState extends State<InProgressView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      await rvm.getCurrentReport(avm.user!, status: "in-progress");
    });
  }
  ...
  Future<void> _getReport() async {
    ...
    if (avm.user != null) {
      rvm.getCurrentReport(avm.user!, status: "in-progress");
    }
  }
      ...
      child: Consumer<ReportViewModel>(
        builder: (context, value, child) {
          return value.isLoading
              ...
              : value.reportProgress == null
              ? Center(child: Text("Tidak ada laporan"))
              : value.reportProgress!["data"].length == 0
              ? Center(child: Text("Tidak ada laporan"))
              : ListView.separated(
                itemBuilder: (context, index) {
                  final data = value.reportProgress!["data"];
                ...
                itemCount: value.reportProgress!["length_progress"],
              );
        },
      ),
    );
